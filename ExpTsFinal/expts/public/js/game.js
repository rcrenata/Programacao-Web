...
function showGameOver() {
  // --- INÍCIO DA LÓGICA PARA SALVAR A PONTUAÇÃO ---
  console.log(`Fim de jogo! Pontuação a ser salva: ${score}`);
  
  // Envia a pontuação para o servidor via AJAX
  fetch('/game/score', {
      method: 'POST',
      headers: {
          'Content-Type': 'application/json',
      },
      body: JSON.stringify({ score: score }) // Envia a pontuação no corpo da requisição
  })
  .then(response => {
      if (response.ok) {
